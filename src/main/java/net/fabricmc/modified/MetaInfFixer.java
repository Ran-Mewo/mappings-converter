...
public class MetaInfFixer implements OutputConsumerPath.ResourceRemapper {
	...
	@Override
	public boolean canTransform(net.fabricmc.modified.TinyRemapper remapper, Path relativePath) {
	...
	@Override
	public void transform(Path destinationDirectory, Path relativePath, InputStream input, net.fabricmc.modified.TinyRemapper remapper) throws IOException {
	...
	private static String mapFullyQualifiedClassName(String name, net.fabricmc.modified.TinyRemapper tr) {
		assert name.indexOf('/') < 0;

		return tr.defaultState.remapper.map(name.replace('.', '/')).replace('/', '.');
	}

	private static void fixManifest(Manifest manifest, net.fabricmc.modified.TinyRemapper remapper) {
