...
package net.fabricmc.modified.extension.mixin.soft.util;
...
public class NamedMappable implements IMappable<String> {
	...
	public String result() {
		...
		Collection<String> collection = targets.stream()
				.map(target -> data.resolver.resolveMember(target, name, desc, net.fabricmc.modified.extension.mixin.common.ResolveUtility.FLAG_UNIQUE | ResolveUtility.FLAG_RECURSIVE))
				.filter(Optional::isPresent)
				.map(Optional::get)
		...
		if (collection.size() > 1) {
			data.logger.error(String.format(net.fabricmc.modified.extension.mixin.common.data.Message.CONFLICT_MAPPING, this.name, collection));
