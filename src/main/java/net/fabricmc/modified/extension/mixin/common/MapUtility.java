...
import net.fabricmc.modified.api.TrRemapper;

public final class MapUtility {
	private final net.fabricmc.modified.api.TrRemapper remapper;
	private final Logger logger;

	public static final List<String> IGNORED_NAME = Arrays.asList("<init>", "<clinit>");

	public MapUtility(net.fabricmc.modified.api.TrRemapper remapper, Logger logger) {
		this.remapper = Objects.requireNonNull(remapper);
		this.logger = Objects.requireNonNull(logger);
	}

	public String mapName(net.fabricmc.modified.api.TrClass _class) {
		return remapper.map(_class.getName());
	}

	public String mapName(net.fabricmc.modified.api.TrMember member) {
		if (member.isField()) {
	...
	public String mapDesc(net.fabricmc.modified.api.TrMember member) {
		if (member.isField()) {
